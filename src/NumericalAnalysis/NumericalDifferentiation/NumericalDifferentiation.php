...
abstract class NumericalDifferentiation
{
    ...
    /** @var int Index of y */
    const Y = 1;
    public static abstract function differentiate($target, $source, ...$args);
    /**
     ...
     * @return array[]
     */
    protected static function functionToPoints(callable $function, $start, $end, $n)
    {
        $points = [];
        $h = ($end - $start) / ($n - 1);
        for ($i = 0; $i < $n; $i++) {
     ...
     */
    public static function validate(array $points, $degree)
    {
    ...
    protected static function sort(array $points)
    {
        usort($points, function ($a, $b) {
            return $a[self::X] < $b[self::X] ? -1 : ($a[self::X] == $b[self::X] ? 0 : 1);
        });
        return $points;
    }
    /**
