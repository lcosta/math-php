...
class Descriptive
{
     ...
     */
    public static function variance(array $numbers, $ν)
    {
        if (empty($numbers)) {
     ...
     */
    public static function weightedSampleVariance(array $numbers, array $weights, $biased = false)
    {
        if (count($numbers) === 1) {
            return 0;
        }
     ...
     * @throws Exception\OutOfBoundsException if degrees of freedom is ≤ 0
     */
    public static function standardDeviation(array $numbers, $SD＋ = false)
    {
        if (empty($numbers)) {
     ...
     * @throws Exception\OutOfBoundsException if degrees of freedom is ≤ 0
     */
    public static function sd(array $numbers, $SD＋ = false)
    {
     ...
     * @throws Exception\BadDataException if the input array of numbers is empty
     */
    public static function quartiles(array $numbers, $method = 'exclusive')
    {
     ...
     * @throws Exception\BadDataException if the input array of numbers is empty
     */
    public static function quartilesExclusive(array $numbers)
    {
        if (empty($numbers)) {
     ...
     * @throws Exception\BadDataException if the input array of numbers is empty
     */
    public static function quartilesInclusive(array $numbers)
    {
        if (empty($numbers)) {
            throw new Exception\BadDataException('Cannot find the quartiles of an empty list of numbers');
        }
     ...
     * @throws Exception\BadDataException if the input array of numbers is empty
     */
    public static function interquartileRange(array $numbers, $method = 'exclusive')
    {
     ...
     * @throws Exception\BadDataException if the input array of numbers is empty
     */
    public static function iqr(array $numbers, $method = 'exclusive')
    {
        return self::quartiles($numbers, $method)['IQR'];
    }
    /**
     ...
     */
    public static function percentile(array $numbers, $P)
    {
        if (empty($numbers)) {
     ...
     */
    public static function describe(array $numbers, $population = false)
    {
        $n = count($numbers);
        $μ = Average::mean($numbers);
        $σ = self::standardDeviation($numbers, $population);
        return ['n' => $n, 'min' => min($numbers), 'max' => max($numbers), 'mean' => $μ, 'median' => Average::median($numbers), 'mode' => Average::mode($numbers), 'range' => self::range($numbers), 'midrange' => self::midrange($numbers), 'variance' => $population ? self::populationVariance($numbers) : self::sampleVariance($numbers), 'sd' => $σ, 'cv' => $μ ? $σ / $μ : \NAN, 'mean_mad' => self::meanAbsoluteDeviation($numbers), 'median_mad' => self::medianAbsoluteDeviation($numbers), 'quartiles' => self::quartiles($numbers), 'midhinge' => self::midhinge($numbers), 'skewness' => $population ? $n > 0 ? RandomVariable::populationSkewness($numbers) : null : ($n >= 2 ? RandomVariable::skewness($numbers) : null), 'ses' => $n > 2 ? RandomVariable::ses($n) : null, 'kurtosis' => RandomVariable::kurtosis($numbers), 'sek' => $n > 3 ? RandomVariable::sek($n) : null, 'sem' => RandomVariable::standardErrorOfTheMean($numbers), 'ci_95' => RandomVariable::confidenceInterval($μ, $n, $σ, '95'), 'ci_99' => RandomVariable::confidenceInterval($μ, $n, $σ, '99')];
    }
    /**
     * Five number summary
