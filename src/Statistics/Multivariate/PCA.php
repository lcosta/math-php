...
class PCA
{
     ...
     */
    public function __construct(Matrix $M, $center = true, $scale = true)
    {
        ...
        $this->center = $center === true ? $this->center = $M->columnMeans() : ($this->center = new Vector(array_fill(0, $M->getN(), 0)));
        if ($scale === true) {
            $scaleArray = [];
            for ($i = 0; $i < $M->getN(); $i++) {
     ...
     * @throws Exception\MathException
     */
    public function standardizeData(Matrix $new_data = null)
    {
        if ($new_data === null) {
            $X = $this->data;
        } else {
     ...
     * @throws Exception\MathException
     */
    public function getScores(Matrix $new_data = null)
    {
        if ($new_data === null) {
     ...
     * @throws Exception\MathException
     */
    public function getQResiduals(Matrix $new_data = null)
    {
        $vars = $this->data->getN();
        if ($new_data === null) {
            $X = $this->data;
     ...
     * @throws Exception\MathException
     */
    public function getT2Distances(Matrix $new_data = null)
    {
        $vars = $this->data->getN();
        if ($new_data === null) {
            $X = $this->data;
     ...
     */
    public function getCriticalT2($alpha = 0.05)
    {
        $samp = $this->data->getM();
        $vars = $this->data->getN();
        $T²Critical = [];
     ...
     * @throws Exception\MathException
     */
    public function getCriticalQ($alpha = 0.05)
    {
        $vars = $this->data->getN();
        $QCritical = [];
        for ($i = 0; $i < $vars - 1; $i++) {
