...
class Set implements \Countable, \Iterator
{
     ...
     * @return Set (this set)
     */
    public function addMulti(array $members)
    {
        foreach ($members as $member) {
            $this->A[$this->getKey($member)] = $member;
        }
     ...
     * @return Set (this set)
     */
    public function removeMulti(array $x)
    {
        foreach ($x as $member) {
     ...
     * @return boolean
     */
    public function isDisjoint(Set $other)
    {
     ...
     * @return boolean
     */
    public function isSubset(Set $B)
    {
        $B_array = $B->asArray();
     ...
     * @return boolean
     */
    public function isProperSubset(Set $B)
    {
        $B_array = $B->asArray();
        $A∩B = array_intersect_key($this->A, $B_array);
        $A∖B = array_diff_key($this->A, $B_array);
     ...
     * @return boolean
     */
    public function isSuperset(Set $B)
    {
        $B_array = $B->asArray();
        $A∩B = array_intersect_key($this->A, $B_array);
     ...
     * @return boolean
     */
    public function isProperSuperset(Set $B)
    {
        $B_array = $B->asArray();
        $A∩B = array_intersect_key($this->A, $B_array);
        $A∖B = array_diff_key($B_array, $this->A);
     ...
     * @return Set
     */
    public function intersect(Set ...$Bs)
    {
        $B_members = [];
        foreach ($Bs as $B) {
            $B_members[] = $B->asArray();
     ...
     * @return Set
     */
    public function difference(Set ...$Bs)
    {
        $B_members = [];
        foreach ($Bs as $B) {
            $B_members += $B->asArray();
