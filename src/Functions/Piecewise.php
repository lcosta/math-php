...
class Piecewise
{
    ...
    public function __construct(array $intervals, array $functions)
    {
        ...
        foreach ($intervals as $interval) {
            ...
            $lastA = isset($a) ? $a : -INF;
            $lastB = isset($b) ? $b : -INF;
            $lastBOpen = isset($bOpen) ? $bOpen : false;
            ...
            $a = $interval[0];
            $b = $interval[1];
            $aOpen = isset($interval[2]) ? $interval[2] : false;
            $bOpen = isset($interval[3]) ? $interval[3] : false;
     ...
     * @throws Exception\BadDataException if an interval cannot be found which contains our $x
     */
    private function getFunction($x)
    {
     ...
     * @return bool
     */
    private function openOpen($aOpen, $bOpen)
    {
        return $aOpen && $bOpen;
    }
    /**
     ...
     * @return bool
     */
    private function openClosed($aOpen, $bOpen)
    {
        return $aOpen && !$bOpen;
    }
    /**
     ...
     * @return bool
     */
    private function closedOpen($aOpen, $bOpen)
    {
        return !$aOpen && $bOpen;
    }
    /**
     ...
     * @return bool
     */
    private function closedClosed($aOpen, $bOpen)
    {
        return !$aOpen && !$bOpen;
    }
    /**
     ...
     * @throws Exception\BadDataException if one interval starts or ends inside another interval
     */
    private function checkAsAndBs($a, $b, $lastA, $lastB, $lastBOpen, $aOpen, $bOpen)
    {
        if ($a === $b && ($aOpen || $bOpen)) {
