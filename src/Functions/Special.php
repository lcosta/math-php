...
class Special
{
     ...
     * @return int
     */
    public static function signum($x)
    {
     ...
     * @throws Exception\OutOfBoundsException
     */
    public static function multivariateBeta(array $αs)
    {
        $∏Γ⟮α⟯ = 1;
        foreach ($αs as $α) {
            $∏Γ⟮α⟯ *= self::Γ($α);
     ...
     */
    public static function generalizedHypergeometric($p, $q, ...$params)
    {
        $n = count($params);
        if ($n !== $p + $q + 1) {
     ...
     * @return array
     */
    public static function softmax(array $𝐳)
    {
        $ℯ = \M_E;
