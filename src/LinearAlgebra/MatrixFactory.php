...
class MatrixFactory
{
     ...
     * @throws Exception\BadDataException
     */
    public static function createFromVectors(array $A)
    {
     ...
     * @throws Exception\OutOfBoundsException if n < 0
     */
    public static function exchange($n)
    {
        if ($n < 0) {
            throw new Exception\OutOfBoundsException("n must be ≥ 0. n = {$n}");
        }
     ...
     * @throws Exception\OutOfBoundsException if n < 0
     */
    public static function downshiftPermutation($n)
    {
     ...
     */
    public static function upshiftPermutation($n)
    {
     ...
     */
    public static function one($m, $n)
    {
        if ($m < 1 || $n < 1) {
            throw new Exception\OutOfBoundsException("m and n must be > 0. m = {$m}, n = {$n}");
        }
     ...
     */
    public static function eye($m, $n, $k, $x = null)
    {
        if ($n < 0 || $m < 0 || $k < 0) {
     ...
     * @throws Exception\MatrixException
     */
    public static function diagonal(array $D)
    {
        $m = count($D);
        $A = [];
        for ($i = 0; $i < $m; $i++) {
     ...
     * @throws Exception\OutOfBoundsException
     */
    public static function hilbert($n)
    {
        if ($n < 1) {
     ...
     * @throws Exception\MatrixException
     */
    public static function vandermonde(array $M, $n)
    {
        $A = [];
        foreach ($M as $row => $α) {
            for ($i = 0; $i < $n; $i++) {
     ...
     * @throws Exception\OutOfBoundsException
     */
    public static function givens($m, $n, $angle, $size)
    {
     ...
     * @throws Exception\MatrixException
     */
    public static function random($m, $n, $min = 0, $max = 20)
    {
        $A = [];
        for ($i = 0; $i < $m; $i++) {
            for ($j = 0; $j < $n; $j++) {
     ...
     */
    private static function checkParams(array $A)
    {
        if (empty($A)) {
     ...
     */
    private static function determineMatrixType(array $A)
    {
        $m = count($A);
        $n = count($A[0]);
