...
class RowEchelonForm extends Matrix
{
     ...
     */
    public function __construct(array $A, $swaps)
    {
     ...
     */
    public static function gaussianElimination(Matrix $A)
    {
        $m = $A->getM();
        $n = $A->getN();
        $size = min($m, $n);
     ...
     * @throws Exception\BadParameterException
     */
    public static function rowReductionToEchelonForm(Matrix $A)
    {
        $m = $A->m;
        $n = $A->n;
        $R = $A;
